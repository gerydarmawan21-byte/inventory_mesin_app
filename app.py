...
if st.session_state.get("login", False):
        ...
        if st.session_state["role"] != "admin":
            ...
            if st.button("Simpan"):
                ...
                if exist:
                    ...
                    session.add(m)
                    session.commit()
                    session.add(History(username=st.session_state["username"], action="Tambah Mesin", mesin_id=m.id))
                    session.commit()
        ...
        if st.session_state["role"] != "admin":
            ...
            if mesin:
                ...
                if st.button("Update"):
                    ...
                    session.commit()
                    session.add(History(username=st.session_state["username"], action="Update Mesin", mesin_id=mesin.id))
                    session.commit()
                ...
                if st.button("Hapus"):
                    ...
                    session.commit()
                    session.add(History(username=st.session_state["username"], action="Hapus Mesin", mesin_id=mesin.id))
                    session.commit()
                ...
                if st.button("Simpan ke Database"):
                    ...
                    for _, row in df.iterrows():
                        ...
                        if exist:
                            ...
                            session.commit()
                            session.add(History(username=st.session_state["username"], action="Update via Upload Excel", mesin_id=exist.id))
                            session.commit()
                        else:
                            m = Mesin(
                            ...
                            session.add(m)
                            session.commit()
                            session.add(History(username=st.session_state["username"], action="Upload Excel", mesin_id=m.id))
                            session.commit()
    ...
    # --- RIWAYAT ---
        ...
        df = pd.DataFrame([(h.id, h.mesin_id, h.username, h.action, h.timestamp) for h in data],
                          columns=["ID", "Mesin ID", "Username", "Aksi", "Waktu"])
        st.dataframe(df)
    ...
    # --- TRANSAKSI ---
        ...
        if sub_menu == "Sewa":
            ...
            if st.button("Proses Sewa"):
                ...
                session.commit()
                session.add(History(username=st.session_state["username"], action=f"Sewa {jumlah}", mesin_id=mesin.id))
                session.commit()
            ...
            if st.button("Proses Kembalikan"):
                ...
                session.commit()
                session.add(History(username=st.session_state["username"], action=f"Kembali {jumlah}", mesin_id=mesin.id))
                session.commit()
            ...
            if st.button("Proses Jual"):
                ...
                session.commit()
                session.add(History(username=st.session_state["username"], action=f"Jual {jumlah}", mesin_id=mesin.id))
                session.commit()
